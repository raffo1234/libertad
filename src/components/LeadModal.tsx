...
import { UNIT_TYPES } from "../constants/types";

type UnitType = "type_1" | "type_2" | "type_3" | "type_4" | "parking" | "";

interface LeadModalProps {
  type?: Exclude<UnitType, "">;
}
...
export default function LeadModal({ type }: LeadModalProps) {
  ...
  function handleClose() {
    ...
    setTimeout(() => {
      ...
      setForm({ name: "", phone: "", email: "", unit_type: type as UnitType });
    }, 400);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const phoneErr = !/^(?:\+?51)?9\d{8}$/.test(form.phone.trim());
    if (nameErr || phoneErr) {
            ...
            <>
              ...
              <div className="flex flex-col gap-7 p-7">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="group transition-[transform,background, scale] relative mt-1 flex h-16 w-full cursor-pointer items-center justify-center gap-2.5 overflow-hidden rounded-full bg-amber-700 font-semibold tracking-wide text-white duration-200 hover:scale-[1.015] hover:bg-amber-700 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-70"
                >
...
function Field({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {children}
      {error && <span className="text-[11px] text-red-500">Campo requerido</span>}
    </div>
  );
}
